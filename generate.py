...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for dom in self.domains:
            for word in self.domains[dom].copy():
                if len(word) > dom.length or len(word) < dom.length:
                    self.domains[dom].remove(word)
        ...
        if self.crossword.overlaps[(x,y)]:
            if self.crossword.overlaps[(x,y)] != None:
                ...
                for wordX in self.domains[x].copy():
                    if not any(wordX[square[0]] == wordY[square[1]] for wordY in self.domains[y]):
